function createPointsMap(directionsArr) {
    ...
    directionsArr.forEach(d => {
        ...
        for (let xp of xPoints) {
            if (xp in pointsMap) {
                pointsMap[xp] = pointsMap[xp].concat(...yPoints)
            } else {
                pointsMap[xp] = yPoints
            }
        }
    })
    return pointsMap
...
function minimizeIntersection(line1, line2) {
    ...
    for (let x in map1) {
        if ([x] in map2) {
            const intersections = map1[x].filter(element => map2[x].includes(element));
            for (let i of intersections) {
                console.log(intersections)
                const manhattanDistance = Math.abs(i) + Math.abs(x)
                min = (manhattanDistance > 0 && manhattanDistance < min) ? manhattanDistance : min
            }
        }
    }
    return min
