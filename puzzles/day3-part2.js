function createPointsMap(directionsArr) {
    const pointsMap = {}
    let x = 0
    let y = 0
    let steps = 0
    directionsArr.forEach(d => {
        const direction = d.charAt(0)
        const distance = parseInt(d.substring(1))
        switch (direction) {
            case 'R':
                for (let i = x + 1; i <= x + distance; i++) {
                    steps++
                    pointsMap[i] = { ...{ [y]: steps }, ...pointsMap[i] }
                }
                x = x + distance
                break
            case 'L':
                for (let i = x - distance + 1; i <= x; i++) {
                    steps++
                    pointsMap[i] = { ...{ [y]: steps }, ...pointsMap[i] }
                }
                x = x - distance
            ...
            case 'U':
                for (let i = y + 1; i <= y + distance; i++) {
                    steps++
                    pointsMap[x] = { ...{ [i]: steps }, ...pointsMap[x] }
                }
                y += distance
                break
            case 'D':
                for (let i = y - distance + 1; i <= y; i++) {
                    steps++
                    pointsMap[x] = { ...{ [i]: steps }, ...pointsMap[x] }
                }
                y -= distance
...
function minimizeIntersection(line1, line2) {
    const map1 = createPointsMap(line1)
    const map2 = createPointsMap(line2)
    let min = Infinity
    for (let x in map1) {
        if ([x] in map2) {
            for (let yPoint in map1[x]) {
                if (map2[x][yPoint]) {
                    const steps = map1[x][yPoint] + map2[x][yPoint]
                    min = (steps > 0 && steps < min) ? steps : min
                }
            }
        }
    }
